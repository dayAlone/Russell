...
export default function* () {
    try {
        let scores = yield Scores.find({
            'share.vk': true,
            user: '56424bc247552511006d77ab',
            created: { $gt: moment('2015-11-22T15:00:00.000+0000').toDate()}
        })
        for (let i = 0; i < scores.length; i++) {
            let {_id, type, scores: s } = scores[i]
            let url = `http://vk.com/share.php?act=count&url=http://russellhobbs-promo.ru/games/${type}/${_id}`
        ...
        for (let i = 0; i < scores.length; i++) {
            let {_id, type, scores: s } = scores[i]
            let url = `https://graph.facebook.com/?id=http://russellhobbs-promo.ru/games/${type}/${_id}`
            let response = yield request(url)
