...
let getStatus = (status) => {
    switch (status) {
    case 'incorrect':
        return { status: 'check_canceled', message: 'Чек не прошел автоматическую проверку' }
    case 'processing':
...
export const addCheck = function* (jar, income) {
    ...
    $('#dataForm tbody tr').map((i, el) => {
        ...
        if (row.organisation.toString() === tmp && parseInt(row.eklz, 10) === parseInt(eklz, 10)) result = row
    })
